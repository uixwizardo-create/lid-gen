...
def rewrite_app_jsx():
    with open('e:/Anti Gravity Project/Lid Gen/frontend/src/App.jsx', 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Insert the pinned sessions state at the top of the component
    # Look for: const [isScraping, setIsScraping] = useState(false);
    state_to_add = """
...
"""
    if "pinnedSessionIds" not in content:
        content = content.replace("  // Scraping progress states", state_to_add + "  // Scraping progress states")
    
    # Find the main return statement
    match = re.search(r'^\s*return \(\s*<div className="min-h-screen', content, re.MULTILINE)
    if not match:
...
"""
    ...
    with open('e:/Anti Gravity Project/Lid Gen/frontend/src/App.jsx', 'w', encoding='utf-8') as f:
